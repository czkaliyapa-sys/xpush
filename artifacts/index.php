...
function create_checkout_session() {
    ...
    // Check for pre-order flag
    $isPreOrder = false;
    if (!empty($input['isPreOrder'])) {
        $isPreOrder = true;
    } elseif (is_array($items)) {
        foreach ($items as $it) {
            if (!empty($it['isPreOrder'])) {
                $isPreOrder = true;
                break;
            }
        }
...
function payments_notify_success() {
    ...
    // Check for pre-order flag (from items or session)
    $isPreOrder = false;
    if (is_array($items)) {
        foreach ($items as $it) {
            if (!empty($it['isPreOrder'])) {
    ...
    // Also check top-level flag
    if (!$isPreOrder && !empty($input['isPreOrder'])) {
        $isPreOrder = true;
    }
    ...
    // Customize subjects based on order type (pre-order, installment, or regular)
    if ($isPreOrder) {
        $subjectCustomer = 'Pre-Order Confirmation — Xtrapush';
        $subjectAdmin = 'New Pre-Order Received — Xtrapush';
    } elseif ($isInstallment) {
        $subjectCustomer = 'Installment Payment Confirmation — Xtrapush';
        $subjectAdmin = 'New Installment Payment — Xtrapush';
    } else {
        $subjectCustomer = 'Payment Confirmation — Xtrapush';
        $subjectAdmin = 'New Payment Received — Xtrapush';
    }
    ...
    try {
        ...
        if ($customerEmail && filter_var($customerEmail, FILTER_VALIDATE_EMAIL)) {
            ...
            // Pre-order specific header and messaging
            $customerHeading = $isPreOrder 
                ? '<h2 style="margin:0 0 8px;color:#111">🎉 Pre-Order Confirmed!</h2>'
                : '<h2 style="margin:0 0 8px;color:#111">Thank you for your payment!</h2>';
            $customerMessage = $isPreOrder
                ? '<p style="margin:0 0 12px;color:#333">Your pre-order has been successfully placed. We will process your order and it will be shipped within <strong>7 business days</strong>. Our team will contact you to confirm delivery details.</p><p style="margin:0 0 12px;color:#f59e0b;font-weight:600">📦 Expected delivery: ~7 days</p>'
        ...
        // Admin heading based on order type
        $adminHeading = $isPreOrder 
            ? '<h2 style="margin:0 0 8px;color:#111">🚨 NEW PRE-ORDER RECEIVED</h2><p style="margin:0 0 12px;color:#f59e0b;font-weight:700;font-size:16px">📦 Pre-order - Customer expects delivery within 7 days</p>'
        ...
        if ($isPaid && $conn && !$conn->connect_errno) {
            ...
            try {
                ...
                if (!$orderId) {
                    ...
                    // Pre-orders get 'pre_order' status instead of regular 'pending'
                    $status = $isPreOrder ? 'pre_order' : ($isInstallment ? 'processing' : 'pending');
                    $pstatus = 'paid';
                    $provider = 'paychangu';
                ...
                // Pre-orders do NOT decrement stock (they're out of stock items)
                $shouldDecrement = !$isPreOrder && (!$isInstallment || ($paymentType === 'installment_deposit'));
...
function payments_paychangu_webhook() {
    ...
    // Check for pre-order flag in meta
    $isPreOrder = !empty($meta['isPreOrder']);
    ...
    if (is_array($sess)) {
        ...
        // Extract pre-order flag from session (takes priority)
        if (!empty($sess['isPreOrder'])) { $isPreOrder = true; }
    }
...
function square_create_checkout_session() {
    ...
    // Check for pre-order flag
    $isPreOrder = !empty($input['isPreOrder']);
    if (!$isPreOrder && is_array($items)) {
        foreach ($items as $it) {
            if (!empty($it['isPreOrder'])) {
                $isPreOrder = true;
                break;
                ...
                if (is_array($store)) {
                    foreach ($store as $ref => $sess) {
                        if (($sess['squareOrderId'] ?? '') === $orderId) {
                            ...
                            $isPreOrder = $sess['isPreOrder'] ?? false;
