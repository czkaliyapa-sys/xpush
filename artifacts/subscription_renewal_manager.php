...
class SubscriptionRenewalManager {
    ...
    private function calculateVisitorStats() {
        ...
        // Match frontend event type 'view_product' (from GadgetDetail.jsx recordEvent call)
        $productViewsQuery = "
            SELECT 
                SUM(CASE WHEN created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY) 
    ...
    private function calculateConversionStats() {
        $query = "
            SELECT 
                ...
                SUM(CASE WHEN event_type IN ('view_product', 'product_view') THEN 1 ELSE 0 END) as product_views,
                SUM(CASE WHEN event_type IN ('add_to_cart', 'addToCart') THEN 1 ELSE 0 END) as add_to_cart,
                SUM(CASE WHEN event_type IN ('checkout_start', 'checkoutStart') THEN 1 ELSE 0 END) as checkout_starts,
