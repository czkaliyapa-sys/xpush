...
// Enhanced Error Handler for Subscription Renewals
class SubscriptionErrorHandler {
    public static function logError($operation, $email, $error, $context = []) {
        $logEntry = [
        ...
        error_log("SUBSCRIPTION_ERROR: " . json_encode($logEntry));
        
        // Also log to file for better tracking
        $logFile = __DIR__ . '/subscription_errors.log';
        file_put_contents($logFile, date('Y-m-d H:i:s') . " - {$operation} - {$email} - {$error}\n", FILE_APPEND);
    }
    
    public static function logSuccess($operation, $email, $details = []) {
        $logEntry = [
            'timestamp' => date('Y-m-d H:i:s'),
            'operation' => $operation,
            'user_email' => $email,
        ...
        error_log("SUBSCRIPTION_SUCCESS: " . json_encode($logEntry));
    }
    
    public static function formatErrorMessage($operation, $email, $error) {
        return sprintf(
            '[%s] %s failed for %s: %s',
            date('Y-m-d H:i:s'),
            $operation,
...
if ($db->connect_error) {
    ...
    SubscriptionErrorHandler::logError('DB_CONNECTION', 'SYSTEM', $db->connect_error);
    exit(1);
}
...
// Find ALL subscriptions (both Square and Paychangu) due within 7 days OR in grace period
$sql = "
    SELECT 
        u.id, 
        u.uid, 
    ...
    WHERE u.subscription_active = 1
      ...
      AND (
        ...
        OR
        -- Grace period expirations (subscriptions that failed payment)
...
echo "🔍 Found $found subscriptions due for renewal or grace period expiration (within next 7 days)\n\n";
...
if ($found > 0) {
    ...
    while ($subscription = mysqli_fetch_assoc($result)) {
        try {
            $processingType = $subscription['subscription_grace_period_end'] ? 'Grace Period Expiration' : 'Normal Renewal';
            echo "  Processing: {$subscription['email']} ({$subscription['tier']}) - Gateway: {$subscription['subscription_payment_gateway']} [$processingType]\n";
            
            if ($subscription['subscription_grace_period_end']) {
                echo "    Grace Period Ends: {$subscription['subscription_grace_period_end']}\n";
            } else {
...
if ($overdueResult && mysqli_num_rows($overdueResult) > 0) {
    ...
    while ($overdue = mysqli_fetch_assoc($overdueResult)) {
        ...
        if (mysqli_query($db, $suspendQuery)) {
            ...
            logSubscriptionEvent($db, $overdue['uid'], $overdue['subscription_id'] ?? 'N/A', 'EXPIRED', 'ACTIVE', 'SUSPENDED', 'Subscription suspended due to overdue payment');
...
function processPaychanguRenewal($db, $subscription) {
    ...
    $operation = 'PAYCHANGU_RENEWAL';
    
    try {
        SubscriptionErrorHandler::logSuccess($operation . '_START', $email, ['tier' => $subscription['tier']]);
        ...
        if ($curlError) {
            ...
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage, ['http_code' => $httpCode]);
            return false;
        }
        ...
        if ($httpCode !== 200 && $httpCode !== 201) {
            ...
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage, ['payload_size' => strlen(json_encode($payload))]);
            return false;
        }
        ...
        if (!$data || !isset($data['data'])) {
            ...
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage, ['response_sample' => substr($response, 0, 200)]);
            return false;
        }
        ...
        if (!$checkoutUrl) {
            ...
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage, ['response_keys' => array_keys($data['data'] ?? [])]);
            return false;
        }
        ...
        if (!sendEmailWithPHPMailer($subscription['email'], $subject, $invoiceHTML)) {
            $errorMessage = "Failed to send renewal email";
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage);
            return false;
        }
        ...
        if (!$updateStmt) {
            $errorMessage = "Failed to prepare update statement: " . $db->error;
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage);
            return false;
        }
        ...
        if (!$updateStmt->execute()) {
            $errorMessage = "Failed to update reminder timestamp: " . $updateStmt->error;
            SubscriptionErrorHandler::logError($operation, $email, $errorMessage);
            $updateStmt->close();
            return false;
        }
        ...
        // Log the renewal initiation event
        logSubscriptionEvent($db, $subscription['uid'], $txRef, 'CREATED', 'ACTIVE', 'PENDING', 'PayChangu renewal checkout created');
        
        SubscriptionErrorHandler::logSuccess($operation, $email, [
            'tx_ref' => $txRef,
            'amount' => $amount,
        ...
        SubscriptionErrorHandler::logError($operation, $email, $errorMessage, ['exception_type' => get_class($e)]);
        return false;
    }
}
...
function processSquareRenewal($db, $subscription) {
    try {
        ...
        if (sendEmailWithPHPMailer($subscription['email'], $subject, $reminderHTML)) {
            ...
            // Log the renewal reminder event
            logSubscriptionEvent($db, $subscription['uid'], $subscription['subscription_id'] ?? 'N/A', 'REMINDER_SENT', 'ACTIVE', 'ACTIVE', 'Square renewal reminder email sent');
...
/**
 * Log subscription events to subscription_history table
 */
function logSubscriptionEvent($db, $userUid, $subscriptionId, $action, $oldStatus, $newStatus, $notes = '') {
    try {
        $stmt = $db->prepare("INSERT INTO subscription_history (user_uid, subscription_id, action, old_status, new_status, notes) VALUES (?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param('ssssss', $userUid, $subscriptionId, $action, $oldStatus, $newStatus, $notes);
            $stmt->execute();
            $stmt->close();
        }
    } catch (Exception $e) {
...
function sendEmailWithPHPMailer($to, $subject, $htmlBody) {
    ...
    if (!class_exists('PHPMailer\\PHPMailer\\PHPMailer')) {
        ...
        $headers = [
            ...
            'From: Xtrapush Gadgets <subscriptions@itsxtrapush.com>',
    ...
    try {
        ...
        // SMTP Configuration (hardcoded - matches index.php defaults)
        $mail->isSMTP();
        ...
        // Email content
        $mail->setFrom('subscriptions@itsxtrapush.com', 'Xtrapush Gadgets');
        $mail->addAddress($to);
